...
The server listens on `http://localhost:8000/mcp` (streamable HTTP transport). All requests require:
...
Use `testcontainers` — Docker must be running locally, but the project stack does not need to be up:
...
MCP_AUTH_TOKEN=ci-dummy \
DATABASE_URL=postgresql+asyncpg://ci:ci@localhost/ci \
  pytest -m "not integration" tests/test_db_audit.py -v
```
...
| mypy | `mypy src/` (dummy env vars) |
| pytest (unit tests only) | `pytest -m "not integration" -v` |

The testcontainers DB suite and `@pytest.mark.integration` live tests are not run in CI. CI must be green before merging.
